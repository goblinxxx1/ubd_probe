...
- **Пошук деградований:** активне відкриття глушать rate-limit/CAPTCHA пошуковиків; **Brave API відкинуто користувачем**; SearXNG зараз віддає **0** результатів — досі в коді, але мертвий. Reframe на seed+пасив **розпочато**: OSM-енумераційний фід (трек 26) дає DDG-незалежне паливо доменів. Лишається **друга половина** — ретайр/вимкнути мертві DDG-залежні левери (query-grid, site:, ActiveDiscovery-провайдер) + вестиж SearXNG (coherence-cleanup).
...
(крос-платформний дедуп) — **закрито** (трек 25). Тріщина #1 (discovery на мертвому DDG) — **половина
закрита**: обрано pivot на seed+пасив, OSM-фід (трек 26) дає DDG-незалежне паливо доменів; лишається
**друга половина** — ретайр/вимкнення мертвих DDG-залежних левер (query-grid, site:, ActiveDiscovery-
провайдер) + вестиж SearXNG (coherence-cleanup). Ще відкрито: атрибуція інертна без людського сіду
(cold-start); асиметрія осей (target курована vs offer self-growing).

**Наступний трек (рекомендація):** усе self-growing discovery + маркетинг-лексикон + **domain-rating**
+ **site:** + **канонічний дедуп** + **OSM-фід** зроблено. Найприродніший наступний — **друга половина
reframe #1**: ретайр/вимкнути мертві DDG-залежні левери + вестиж SearXNG (coherence-cleanup, робить код
і дефолти відповідними реальності). P3 (необовʼязкові): бренд-якорні запити; LLM-хвіст перефразувань
(офлайн, injection-hardened). Деталі — [[ubd-crawler-discovery-scaling-brainstorm]].
