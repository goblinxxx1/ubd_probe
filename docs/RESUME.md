...
3. **Публічний фронтенд** (Vue 3 SPA + Less) — наступний.
4. Crawler (сервіс за кроном).
...
### Стартовий промпт для треку 3 (публічний фронтенд)

```
Продовжуємо проєкт UBD (тека D:\ubd_probe, гілка main — бекенд і адмінка вже тут).
Починаємо під-проєкт 3 — публічний фронтенд: Vue 3 SPA з CSS-препроцесором Less,
показує опубліковані оффери для УБД з фільтрами. Це чиста JavaScript (без TypeScript),
як і адмінка. Створи фіча-гілку feat/public від main і почни з брейнсторму.
```

### Стартовий промпт для треку 4 (crawler) — пізніше
