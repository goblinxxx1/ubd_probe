...
- **Database (app runtime):** MySQL via `DATABASE_URL` env var. Models use portable column types only (no MySQL-only types) so the test suite can run on in-memory SQLite.
- **Tests:** pytest + FastAPI `TestClient`; test DB is in-memory SQLite created fresh per test via fixtures (fast TDD cycle, no MySQL server required to run tests).
...
**Interfaces:**
- Produces:
  - `app.core.config.Settings` with fields `database_url: str`, `test_database_url: str = "sqlite+pysqlite:///:memory:"`, `jwt_secret: str`, `jwt_algorithm: str = "HS256"`, `jwt_expire_minutes: int = 720`, `crawler_api_key: str`, `seed_admin_email: str`, `seed_admin_password: str`. Instance `settings`.
  ...
  - `tests/conftest.py` fixtures: `db_session` (fresh in-memory SQLite, all tables created), `client` (TestClient with `get_db` overridden to use `db_session`).
...
```
DATABASE_URL=mysql+pymysql://ubd:ubd@localhost:3306/ubd
...
Run: `cd backend && DATABASE_URL="sqlite+pysqlite:///./tmp_autogen.db" alembic revision --autogenerate -m "initial schema"`
Expected: a new file under `alembic/versions/` containing `create_table` for all seven tables + two association tables. Delete `tmp_autogen.db` afterward. Inspect the file to confirm all tables/columns are present.
...
- **Test DB note:** models use portable types so the SQLite-based test harness matches MySQL behavior for the covered features; the runtime target remains MySQL via `DATABASE_URL`.
