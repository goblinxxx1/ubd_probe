...
- Produces:
  ...
  - `discovery.passive.extract_source_candidates(item: RawItem, known: set[str]) -> list[SourceCandidate]` — finds telegram/instagram/facebook handles in `item.text` + `item.links`, skips any whose normalized ref is in `known`. (Generic external website links are intentionally out of scope — too low-signal/noisy; social handles are deliberate high-signal mentions.)
...
Expected: FAIL — `crawler.wiring` missing.
...
- [ ] **Step 6: Create the scheduler script and README**
...
- [ ] **Step 7: Full crawler suite green**
