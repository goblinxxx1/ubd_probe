...
- узяти top-1 UA-бізнес-домен з порогом впевненості (overlap токенів
  назви ↔ домен/тайтл);
- **R1 (проти омонімів/однойменних):** для КОРОТКОЇ/генеричної назви (≤2 значущі
  токени, напр. «Планета Фітнес») місто **обовʼязкове** — якщо `city is None`,
  повертаємо `None` (не вгадуємо навмання). Для довшої унікальної назви місто —
  бонус до впевненості, не обовʼязкове.
...
основного harvest-пасу зі своїм бюджетом.

**R3 (спільний пошуковий ресурс — єдине місце побічного впливу на основний):**
підпошук ділить ті самі DDG/SearXNG-бекенди, тож:
- **budget-cap:** не більше `subsearch_search_budget` пошуків за пас (конфіг,
  дефолт напр. 15) — решта черги відкладається/дропається;
- **backoff-aware:** якщо scheduler під бекофом ([[ubd-crawler-backoff-scheduler]]),
  підпошук **пропускається цілком** цього пасу (основний пошук у пріоритеті);
- реюз наявного `SearchCache` ([[ubd-ddg-antithrottle]]) — повтори не б'ють мережу.

Для кожного `(name, city)` з черги (в межах budget-cap):
...
- домен є → **власний фетч+екстракт** цим підпошуком (реюз наявних
  ...
  витягти офери; знайдені → `api.submit_offer` → офер лягає у **ту саму загальну
  чергу модерації** (`pending_review`), що й усі інші офери. Жодної окремої черги,
  жодної авто-публікації; атрибуція = бізнес (сайт бізнесу, не каталог).
...
- **R3 — спільний пошуковий ресурс під контролем:** budget-cap на пошуки/пас +
  повний skip під бекофом (основний пошук у пріоритеті) + `SearchCache`; підпошук
  не може «зʼїсти» рейт-ліміт основного краулу.
...
- **R1:** генерична назва (≤2 токени) без міста → `None`; з містом → домен.
- **R3:** черга > budget → не більше N пошуків; під бекофом → 0 пошуків (skip).
