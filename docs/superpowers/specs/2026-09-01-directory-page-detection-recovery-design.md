...
### 5. Інʼєкція в пошук (автономно), один раз

- назви бізнесів беруться **ЛИШЕ** з каталог-сторінок (п.2); більше нізвідки.
- **один раз шукаємо:** знайдений домен → **наявний `aggregator_store`**
  (`AggregatorDomainStore.add`) — він re-surface'ить бізнес-домен як
  website-`SourceCandidate`, і краулер фетчить його автономно (той самий шлях,
  що osm/brand/aggregator-фіди). Реюз = нуль нового crawl-коду.
- dedupe в межах ОДНОГО проходу — простий in-memory set назв (не шукати той
  самий бізнес двічі за пас). **БЕЗ персистентного стору-памʼяті** — нічого не
  накопичуємо (див. п.6 «забуваємо»).

### 6. Нема офера → видалити домен і забути

- recovery-домени позначаємо origin у самому `aggregator_store` (запис
  `{host, origin: "catalog_recovery"}`), щоб відрізняти від органічно-нахарвестених;
- після краулу такого домену, якщо `domain_registry` фіксує empty-pass
  (0 оферів) → `aggregator_store.remove(host)` (нова дрібна функція): домен
  **видаляється з пошуку і забувається** — жодного негативного memo, жодного
  сліду. Наступного разу нічого не памʼятаємо про цей бізнес.
- органічно-нахарвестені агрегатор-домени (не recovery-origin) поведінки не
  міняють — лишаються під наявним empty-skip.
- `AggregatorDomainStore.remove(host)`: видалити зі списку, скоригувати cursor.

> Свідомий tradeoff: «забуваємо» означає, що коли бізнес ДОДАСТЬ знижку пізніше,
> ми не відстежуємо це проактивно (його підхопить звичайне discovery, якщо сайт
> десь спливе органічно). Натомість — 0 накопиченого сміття, як ти просив.
...
- `aggregator_store.remove`: додати→видалити, cursor коректний; empty-pass hook
  видаляє лише recovery-origin домен (органічні — недоторкані).
- «один раз» у межах проходу: та сама назва двічі за пас → один resolve;
