...
`_passive_workers` — новий інʼєктований параметр `Runner.__init__`, дефолт `1`
(back-compat: `1` → byte-identical серійна поведінка).
...
Нова ручка `passive_workers` (env → `config` → `wiring` → `Runner`), дефолт
консервативний (пропозиція `4`). `1` = точна наявна серійна поведінка
(kill-switch без коду). Пул створюється лише в `run_passive`; активний прохід і
scheduler нічого не бачать.
