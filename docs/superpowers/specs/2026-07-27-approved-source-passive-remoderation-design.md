...
- **Reject shadow, потім знову зʼявляється ТА САМА зміна (той самий `content_hash`).** `UniqueConstraint(source_id, content_hash)` тримає рівно один рядок на пару → нового pending-shadow фізично бути не може. Гілка 1 матчить наявний `rejected`-рядок (він **не** `expired`, тож не пропускається) і тихо бампає `last_seen`, повертаючи його. Тобто **reject фінальний для цього конкретного контенту** — модератор сказав «ні», система не нагадує повторно. *(Це свідома поведінка, підкріплена unique-constraint; статус-фільтр у гілці 1 тут відкрив би IntegrityError.)*
- **Реверт знижки після апруву (10→20→approve→10).** Після апруву 20-офер `published`, старий 10-parent `expired` (тримає `content_hash` h1). Реверт на 10: гілка 1 **ігнорує expired-рядки** → не короткозамикає на мертвому h1 → падає в гілку 2, яка **реанімує** expired-рядок (S,h1) у shadow, що supersede-ить живий 20-офер (обхід unique-constraint без INSERT). Модератор апрувить → 20 гасне, 10 знову `published`. Жива картка не зникає. *(Ключ: гілка 1 фільтрує `status != expired`; будь-який знайдений гілкою 2 (S,h)-рядок гарантовано expired, бо не-expired перехопила б гілка 1 → реанімація безпечна.)*
- **Ацикл supersede-графа.** При публікації офера його `supersedes_offer_id` очищається (після використання для гасіння parent). Тож published-рядок не має вихідного supersede-ребра, граф завжди глибини 1 (`pending → published → null`), і реанімація колишнього parent не утворює 2-вузловий FK-цикл (який `selectin`-eager-load флашив би як `CircularDependencyError`). Побічно: published-офери не мають supersede → admin-маркер не тече у вкладку «Опубліковані».
