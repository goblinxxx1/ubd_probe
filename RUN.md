...
### Варіант C — Живий фронт-кодинг (dev-фронти на Docker-бекенд) ⭐

Найзручніше для ітерації UI: **бекенд+БД+краулер лишаються в Docker** (Варіант A, прод-стек
недоторканий на `:8080`/`:8082`), а на хості піднімаєш **лише Vite dev-фронти** з HMR. Вони бʼють
`/api` у той самий Docker-бекенд (`:8000`), тож працюєш із **живими даними** (та сама БД/черга).

Передумова: Docker-стек піднятий (`docker compose up -d`), бекенд віддає `:8000`.

```bash
# два окремі термінали (з кореня репо):
npm --prefix admin run dev     # http://localhost:5173  (адмінка, HMR, /api → :8000)
npm --prefix public run dev    # http://localhost:5174  (публічний сайт, HMR, /api → :8000)
```

Редагуєш `admin/src/**` або `public/src/**` → зміни в браузері миттєво (без ребілду).
Коли готово — деплой на прод: `docker compose build admin public && docker compose up -d`.

> Порти/проксі задані в `admin/vite.config.js` (5173) і `public/vite.config.js` (5174).
> Є також `.claude/launch.json` (конфіги `admin-dev` / `public-dev`) для launch-механізму.
> Dev-фронти НЕ замінюють прод-стек — це паралельний канал лише для перегляду правок.
