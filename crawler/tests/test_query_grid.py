...
def test_grid_size_is_base_plus_geo_block():
    base = len(INTENT_FORMS) * len(AUDIENCE_FORMS)          # 14*25 = 350
    geo = len(GEO_INTENTS) * len(GEO_AUDIENCES) * len(GRID_CITIES)  # 5*6*45 = 1350
    grid = build_grid()
    assert base == 350 and geo == 1350
    assert len(grid) == base + geo == 1700


def test_base_prefix_is_byte_stable():
    # first 350 == the plain intent×audience grid, unchanged order
    grid = build_grid()
    plain = build_grid(cities=[])
    assert len(plain) == 350
    assert grid[:350] == plain
...
def test_geo_block_present_and_ordered():
    ...
    assert grid[350].endswith(f" {GRID_CITIES[0]}")
    assert grid[350] == f"{GEO_INTENTS[0]} {GEO_AUDIENCES[0]} {GRID_CITIES[0]}"
...
def test_cities_di_controls_geo_size():
    ...
    assert len(grid) == 350 + len(GEO_INTENTS) * len(GEO_AUDIENCES) * 2   # 350 + 60
...
def test_services_block_appended_after_geo():
    ...
    base = build_grid()                       # 1700, no services
