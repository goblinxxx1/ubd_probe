...
class ApiClient:
    ...
    def submit_query_candidates(self, items: list[dict]) -> dict:
        r = self._client.post("/api/internal/query-terms", json={"candidates": items})
        r.raise_for_status()
        return r.json()

    def list_approved_query_terms(self) -> list[str]:
        r = self._client.get("/api/internal/query-terms/approved")
        r.raise_for_status()
        return r.json()

    def list_rejected_query_terms(self) -> list[str]:
        r = self._client.get("/api/internal/query-terms/rejected")
        r.raise_for_status()
        return r.json()
